...
function buildCtxQuery(ctx) {
  ...
  params.set('from', String(ctx.from ?? '2024'));
  params.set('to', String(ctx.to ?? '2030'));
...
async function init() {
  ...
  /* 주소창 → data-* Fallback 주입 (+출처 플래그 qs) */
  {
    ...
    if (root && !root.dataset.from && urlp.get('from')) {
      root.dataset.from = urlp.get('from');
    }
    if (root && !root.dataset.to && urlp.get('to')) {
      root.dataset.to = urlp.get('to');
    }
  }
  ...
  // 데이터 로드 (더미/실데이터 분기)
  const data = useDummy
    ? makeDummyForecast(ctx.from, ctx.to)
...
function makeDummyForecast(fromYear, toYear) {
  ...
  // to가 없으면 from 기준으로 7년 구간
  if (!Number.isFinite(to))   to   = from + 6;

  // 2) 범위 보정
  if (to < from) [from, to] = [to, from];
  if (to === from) to = from + 6; // ✅ 같으면 7년 구간으로 확장
...
async function fetchForecast(ctx) {
  // 1) from/to 보정(같으면 7년 확장)
  let from = parseInt(String(ctx.from ?? '2024'), 10);
  let to = parseInt(String(ctx.to ?? '2030'), 10);
  if (!Number.isFinite(from)) from = 2024;
...
function fallbackDefaultContext(root) {
  ...
  // from/to: URL → data-* → 기본값(now ~ now+6)
  let from = parseInt(urlp.get('from') || root?.dataset.from || String(now), 10);
  let to   = parseInt(urlp.get('to')   || root?.dataset.to   || String(now + 6), 10);
  ...
  // builtYear: URL → data-* → (from - 13) 정도, 양수 보장
  let builtYear = parseInt(urlp.get('builtYear') || root?.dataset.builtYear || String(from - 13), 10);
